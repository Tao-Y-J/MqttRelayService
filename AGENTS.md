# MqttRelayService Codex Working Agreement

这个仓库面向 `.NET 8 Worker Service + MQTTnet Broker` 的长期演进，文档、上下文和代码都要按“可持续维护”来组织。

## 主要目标

帮助仓库在单机 MQTT Broker、转发队列、重试、死信、Windows Service 部署和可观测性这些方向上稳定迭代，同时避免把业务实现写散、写乱。

## 默认工作流

按变更规模选择工作方式。

### 小改动

适用场景：

- 文档补充
- 配置说明
- 单个文件的小修正
- 明显的注释或说明修订

执行顺序：

1. 先确认目标文件和影响范围。
2. 直接做最小必要修改。
3. 做最小但足够的可读性或一致性检查。
4. 交付时说明改了什么、验证了什么、还有什么没验证。

### 中改动

适用场景：

- 跨 2 到 6 个文件的改动
- 需要同步更新上下文目录、代码地图或历史记录
- 需要说明结构变化、运行边界或可靠性边界

执行顺序：

1. 先写一份简短实现说明或上下文说明。
2. 说明目标、影响面、关键决策、验证方式。
3. 再分批修改。
4. 改完后回写相关上下文文档。

### 大改动

适用场景：

- 解决方案级调整
- Broker、队列、重试、死信、安装方式这类核心链路变化
- 需要明确可靠性边界或部署边界的改动

执行顺序：

1. 先写设计或实现说明。
2. 明确不做什么，避免范围漂移。
3. 分批实施，每批都做一次验证。
4. 最后把结构事实沉淀回仓库地图和历史记录。

## 验证顺序

优先顺序如下：

1. 先验证最受影响的工程或入口。
2. 再验证真实运行路径，而不是只看静态文件。
3. 文档任务则先做一致性检查，再做可读性检查。

## 仓库级要求

- 本仓库当前由主线程和文档线程并行工作，看到别人的修改不要回退，先适配现有状态。
- 默认只做最小必要修改，不把顺手重构混进当前任务。
- 这个仓库的主体是 MQTT Worker Service，不是 UI 项目；所有上下文、文档和示例都要围绕 Broker、订阅、消息生命周期、重试和死信来写。
- MQTT 事件回调必须保持轻量，耗时工作要下沉到内部队列和后台消费者。
- 队列必须有容量上限，不能出现无界内存增长。
- 所有后台入口都要顶层捕获异常，不能让未处理异常直接停止 Host。
- 如果第一版没有磁盘队列，README 和上下文文档都要明确说明可靠性边界。

## 文本编码要求

- 除仓库内已有明确例外规则的文件类型外，所有文本文件的读写默认使用 `UTF-8 with BOM`。
- 当前已确认的例外是 `.cmd` / `.bat`，这两类文件必须保持 `GBK (936)`，避免 Windows cmd 下中文乱码和命令解析异常。
- `.ps1`、`.md`、`.json`、`.yml`、`.xml`、`.cs` 等文本文件默认按 `UTF-8 with BOM` 处理。
- 新增或修改中文文本后，要同时检查编码和可读性，确认没有乱码。

## 中文注释要求

- 新增或修改的公共类、接口、核心模型，默认要补中文注释。
- 关键流程方法需要补中文注释，尤其是 Broker 启停、事件处理、入队出队、路由、重试、死信和优雅停机。
- 注释要解释职责、边界和设计意图，不做逐行翻译。

## 文档协作

- 每次新任务先读 `AGENTS.md`、`lessons.md` 和 `.codex/documents/01-长期有效规范/仓库上下文管理入口.md`。
- 需要判断影响面时，再看 `.codex/documents/02-仓库地图/全仓库代码地图.md`。
- 中改动和大改动默认在 `.codex/documents/03-历史任务记录/` 留一份说明。
- 结构变化写回代码地图，长期经验写回 `lessons.md`，一次性任务写回历史记录。

## 文档质量要求

- 历史任务记录、代码地图、设计决策文档中，**只允许描述实际实现的方案**，不允许出现"方案A/方案B对比"、"替代方案对比"、"可考虑"、"也许"、"可能"等假设性或模棱两可的描述。
- 如果必须提及未采用的方案，只能作为一句话脚注说明"未采用X，因为Y"，且不得展开优缺点对比。
- 所有技术决策必须写清楚：实际做了什么、为什么这么做、边界条件是什么。不允许写"未来可以..."、"后续可以优化..."这类无承诺的推测。
- 后续文档生成必须遵守本条规则，不得产生任何模棱两可的表述。

## 完成检查

结束前至少说明四件事：

1. 改了什么
2. 验证了什么
3. 没验证什么
4. 还有什么风险或后续建议
